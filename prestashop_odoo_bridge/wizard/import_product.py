...
class ImportPrestashopProducts(models.TransientModel):
    ...
    def create_category_feed(self,prestashop,channel_id,categ_id):
        ...
        match = channel_id.match_category_feeds(store_id = categ_id)
        if match :
            match.write(categ_data)
        else:
    ...
    def _prestashop_create_product_feed(self, prestashop, product_id):
        ...
        if product:
            ...
            match = self.channel_id.match_product_feeds(store_id = product_id)
            if match:
                if match.feed_variants:
