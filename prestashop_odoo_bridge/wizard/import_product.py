...
class ImportPrestashopProducts(models.TransientModel):
    ...
    def _get_product_basics(self, prestashop, products):
        if prestashop:
            ...
            name = products.get("name").get("language")[0].get("value")
            ...
            if categ_ids:
                if type(categ_ids)==list:
                    category_ids = [i['id'] for i in categ_ids]
                else:
                    category_ids = [categ_ids['id']]
            ...
            description_sale = products.get("description_short").get("language")[0].get("value")
            description_purchase = products.get("description").get("language")[0].get("value")
        ...
        if prestashop:
            ...
            for value in product_option_value_ids:
                ...
                vals = dict(
                            ...
                            value = product_value.get("name").get("language")[0].get("value"),
                ...
                if type(product_value['name']['language'])==list:
                    for attr_name in product_value['name']['language']:
                        if attr_name['attrs']['id'] == self.channel_id.ps_language_id:
                            value = attr_name['value']
                else:
                    value = product_value.get('name')['language']['value']
                vals["value"] = value
        ...
        if prestashop:
            ...
            if combinations:
                ...
                for combination_id in combination_ids:
                    ...
                    if type(product_option_value_id) == list:
                    ...
                    vals = dict(
                            ...
                            list_price = combination.get("price"),
    ...
    def _prestashop_create_product_feed(self, prestashop, channel_id, product_id):
        if prestashop:
            vals = {}
            try :
            ...
            if product:
                ...
                variants = self._get_feed_variants(prestashop, product)
    ...
    def _filter_product_using_date(self, prestashop, channel, product_import_date):
        if prestashop:
            vals_list = []
            date = fields.Datetime.to_string(product_import_date)
            try:
                data =  prestashop.get('products',  options={'filter[date_add]':'>['+date+']', 'date':1})
                data = data.get("products",{}).get("product")
            except Exception as e:
    ...
    def get_product_all(self, prestashop):
        if prestashop:
            vals_list = []
            products = prestashop.get("products")
            products = products.get("products").get("product")
    ...
    def import_now(self):
        ...
        product_import_date = self._context.get('prestashop_import_date_from')
        if prestashop_object_id:
            vals = self.get_product_by_id(prestashop, prestashop_object_id)
            if vals:
                data_list.append(vals)
        elif product_import_date:
            data_list = self._filter_product_using_date(prestashop,channel,product_import_date)
        else:
            data_list = self.get_product_all(prestashop)
        return data_list