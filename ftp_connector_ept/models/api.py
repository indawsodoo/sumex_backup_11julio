...
class UnicodeDictReader(csv.DictReader):
    ...
    def __next__(self):
        if sys.version_info[0] < 3:
            csv_rv = csv.DictReader.__next__()(self)
            return dict((
                (k, v.decode(self.encoding, 'ignore') if v else v) for k, v in list(csv_rv.items())
            ))
        elif sys.version_info[0] >= 3:
            csv_rv = csv.DictReader.__next__(self)
            return dict((
                (k, v if v else v) for k, v in list(csv_rv.items())
            ))
...
class UnicodeDictWriter(csv.DictWriter):
    ...
    def _dict_to_list(self, rowdict):
        csv_rv = csv.DictWriter._dict_to_list(self, rowdict)
