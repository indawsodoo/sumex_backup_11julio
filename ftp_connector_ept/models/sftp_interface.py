...
class sftp_interface(object):
    ...
    def __enter__(self):
        ...
        try:
            self.ssh.connect(hostname=self.host, port=self.port or 2222, username=self.user, password=self.passwd,
    ...
    def push_to_ftp(self, filename, local_file):
        ...
        try:
            ...
            message = "Exception occur while uploading the file to the FTP, Exception: [{}] or Invalid directory " \
                      "name. ".format(error)
    ...
    def archive_file(self, filenames):
        ...
        for filename in filenames:
            ...
            file_name = "%s_%s" % (filename.split('.')[0], datetime.now().strftime('%Y_%m_%d_%H_%M_%S.csv'))
            toname = "%s/%s" % (self.archive_dir, file_name)
            self.sftp_client.chdir(self.archive_dir)
            try:
                self.sftp_client.rename(fromname, toname)
            except Exception as error:
                message = "Getting Exception while archiving the files, " \
                          "Exception: {}, Download Directory: [{}], Archive Directory: [{}]".format(error,
                                                                                                    self.download_dir,
                                                                                                    self.archive_dir)
                _logger.info(message)
        return message
